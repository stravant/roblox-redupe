...
function TransformHandlesImplementation:updateDrag(globalTransform)
	...
	if self:_shouldSolveConstraints() then
		ret = self:_transformInverseKinematics(globalTransform)
	else
		ret = self:_transformGeometric(globalTransform)
	end
	self._draggerContext.DragUpdatedSignal:Fire(self._initialSelectionInfo)
	return ret
end
