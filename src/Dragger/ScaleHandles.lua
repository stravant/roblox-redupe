...
function ExtrudeHandle:_axesToScale(normalId)
...
function ExtrudeHandle:_updateExtrudeMode()
	...
	self._minimumSize = Vector3.zero
	self._maximumSize = Vector3.new(math.huge, math.huge, math.huge)
...
function ExtrudeHandle:mouseDrag(mouseRay)
	...
	self._lastDeltaSize, self._lastOffset = deltaSizeToApply, localOffsetToApply
	-- 	self._implementation:updateScale(deltaSizeToApply, localOffsetToApply)
	self._props.ApplyScale(deltaSizeToApply, localOffsetToApply)
