...
local DraggedPivot = require(script.Parent.DraggedPivot)
...
function MoveHandles:render(hoveredHandleId)
	...
	if self._draggingHandleId and self._handles[self._draggingHandleId] then
		...
		children[self._draggingHandleId] = Roact.createElement(MoveHandleView, {
			...
			Hovered = false,
			DoubleHandle = handleProps.DoubleHandle,
		})
...
function MoveHandles:mouseDown(mouseRay, handleId)
	...
	if self._handles[handleId] then
		...
		local handleOffset, handleLength =
			MoveHandleView.getHandleDimensionForScale(handleProps.Scale, self._props.Outset, handleProps.FixedOutset, handleProps.DoubleHandle)
...
function MoveHandles:_solveForAdjustedDistance(unadjustedDistance)
	...
	local doubled = self._handles[self._draggingHandleId].DoubleHandle
	local fixedOutset = self._handles[self._draggingHandleId].FixedOutset
	...
	local function getHandleFracForDistance(distance)
		...
		local handleOffset, handleLength =
			MoveHandleView.getHandleDimensionForScale(scale, self._props.Outset, fixedOutset, doubled)
	...
	local function getHandleLengthForDistance(distance)
		local _, handleLength = MoveHandleView.getHandleDimensionForScale(
				getScaleForDistance(distance), self._props.Outset, fixedOutset, doubled)
		return handleLength
	end
...
function MoveHandles:_getSnappedDelta(delta)
	...
	local function snapWithBaseAdjustment(delta, snapAmount: number?)
		local axis = self._handles[self._draggingHandleId].LocalAxis
		local baseOffset = self._draggerContext.StartCFrame:ToObjectSpace(self._draggerContext.StartDragCFrame)
		local baseDistance = (baseOffset.Position * axis).Magnitude
		delta += baseDistance
		if snapAmount then
			delta = math.floor((delta / snapAmount) + 0.5) * snapAmount
		else
			delta = self._draggerContext:snapToGridSize(delta)
		end
		delta -= baseDistance
		return delta
	end

	if self._draggerContext.UseSnapSize then
		local primaryAxis = self._draggerContext.PrimaryAxis
		local axis = self._handles[self._draggingHandleId].LocalAxis

		-- Apply the size based snapping on the first drag or any
		-- subsequent ones on the primary axis.
		if not primaryAxis or primaryAxis:FuzzyEq(axis) then
			local snapAmount = (axis * self._draggerContext.SnapSize).Magnitude
			return snapWithBaseAdjustment(delta, snapAmount)
		end
	end
...
function MoveHandles:mouseDrag(mouseRay)
	...
	local actualGlobalTransform = CFrame.new(self._axis * snappedDelta)
...
function MoveHandles:mouseUp(mouseRay)
	...
	if self._lastGlobalTransformForRender then
		self._props.ApplyTransform(self._lastGlobalTransformForRender)
	end
...
function MoveHandles:_updateHandles()
	if self._selectionInfo:isEmpty() then
		...
		local primaryAxis = self._draggerContext.PrimaryAxis
		local doubleAll = (primaryAxis == nil)
